...
### Changed
- `AVAILABILITY.md` describes capacity as it actually behaves. It said capacity applied to service schedules only and was ignored for staff, and that an empty capacity meant unlimited — the Control Panel wording was corrected in 1.4.6 but the guide never was, and 1.5.0 then extended capacity to employee slots without it. It now covers per-employee seats, the largest bookable party, whole-day services, and the fact that an empty capacity means one booking per slot rather than unlimited.
...
- **A multi-day service linked with `?serviceId=` rendered as an appointment.** The wizard matched the preselected id against the loaded service list with `===`, but a deep link supplies it as a string while the API returns numbers, so the lookup missed and the service fell back to a bare `{ id }` carrying no `durationType`, `price` or `pricingMode`. A whole-day service therefore lost its date-range picker and offered time slots one *minute* long — 480 of them across a 09:00–17:00 day — and per-unit pricing had nothing to price. Ids are now normalised wherever they enter, so `selectService()`, `selectLocation()` and `selectEmployee()` accept either form; `_applyDeepLinks()` already did this for location and employee, and the service was the one that was missed.
- **The booking wizard never knew which services have add-ons.** The service list filtered on `booked_service_extras.enabled`, a column that has never existed — ServiceExtra is an element, so its enabled state lives on `elements`. The query threw on every request, was swallowed into a warning, and left every service reporting no add-ons. Status now comes from the element query, and a disabled add-on correctly stops flagging its service.
...
- `CapacityService::isQuantityAllowed()` no longer loads the service. It tested `$service->allowQuantitySelection` and `$service->maxCapacity`, neither of which has ever existed as a property, a column or a Control Panel field, so both branches were unreachable and the method could only ever reject a quantity below one. How many seats a booking may take belongs to the slot and is enforced by `hasAvailableCapacity()`; the dead branches and the per-call database lookup they needed — one on every availability check — are gone. The signature is unchanged.
