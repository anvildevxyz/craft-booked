...
async function stubApi(page: Page) {
  await page.route('**/booked/api/v1/**', (route) => {
...
test('wizard review + success render correctly (#75, #76, #77)', async ({ page }) => {
  ...
  await page.waitForFunction(() => {
    const el = document.querySelector('[data-booked-wizard]') as any;
    return el?.__bookedController?.wizard?.stepId === 'datetime';
  });

  // Drive through the real core to the review step (bypassing the seed-less calendar).
  await page.evaluate(async () => {
    const wizard = (document.querySelector('[data-booked-wizard]') as any).__bookedController.wizard;
  ...
  const dtText = (key: string) =>
    page.evaluate((k) => {
      const dd = document.querySelector(`[data-booked-step="review"] [data-booked-summary="${k}"]`);
  ...
  await expect(page.locator('[data-booked-summary="status"]')).toHaveText('Confirmed');
});
