...
const slot = (page: Page) => page.locator(`[data-booked-time="${TIME}"]`);
...
test.describe('schedule capacity in the booking wizard', () => {
  ...
  test('offers every seat on an unbooked day', async ({ page }) => {
    ...
    await expect(slot(page)).toHaveAttribute('data-booked-capacity', String(CAPACITY));
  ...
  test('shows the remaining seats on a group slot', async ({ page }) => {
    ...
    // in a data attribute.
    await expect(slot(page).locator('[data-booked-slot-seats]')).toHaveText(`${CAPACITY - 1} available`);
  });
  ...
  test('leaves one-on-one slots unannotated', async ({ page }) => {
    ...
    await expect(slot(page)).toHaveAttribute('data-booked-capacity', '1');
  ...
  test('keeps the slot bookable after a single booking', async ({ page }) => {
    ...
    await expect(slot(page)).toHaveCount(1);
    await expect(slot(page)).toHaveAttribute('data-booked-capacity', String(CAPACITY - 1));
  });
  ...
  test('reveals the quantity picker while several seats remain', async ({ page }) => {
    ...
    const picker = page.locator('[data-booked-slot-quantity]');
    await expect(picker).toBeVisible();

    await picker.locator('[data-booked-action="qty-increment"]').click();
    await expect(picker.locator('[data-booked-slot-qty-value]')).toHaveText('2');
  });
