...
const REGION = `
  <section data-booked-step="payment">
    <p data-booked-payment-status></p>
    <p data-booked-payment-error hidden></p>
    <div data-booked-payment-element></div>
    <button data-booked-action="pay" disabled></button>
  </section>`;
...
function mountRegion() {
  ...
  return document.querySelector('[data-booked-step="payment"]');
}
...
describe('createPaymentStep — mount', () => {
  it('creates the payment, mounts the Payment Element, and enables the pay button', async () => {
    ...
    expect(region.querySelector('[data-booked-action="pay"]').disabled).toBe(false);
    expect(region.querySelector('[data-booked-payment-error]').hidden).toBe(true);
  });
  ...
  it('shows an error and leaves pay disabled when the payment cannot be created', async () => {
    ...
    expect(region.querySelector('[data-booked-action="pay"]').disabled).toBe(true);
  });
});
